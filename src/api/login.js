...
export function getQrcodeKey() {
    ...
    return get(`/login/qr/key?timestamp=${timeStamp}`)
}
...
export function getQrImg(key) {
    const timeStamp = new Date().getTime()
    return get(`/login/qr/create?key=${key}&timeStamp=${timeStamp}&qrimg=1`)
}
...
export function getQrcodeStatus(key) {
    const timeStamp = new Date().getTime()
    return get(`/login/qr/check?key=${key}&timeStamp=${timeStamp}`)
}
